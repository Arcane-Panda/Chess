...
vector<int> possibleMovescX;
vector<int> possibleMovescY;
...
bool check(bool white){
  ...
  possibleMovescX.clear();
  possibleMovescY.clear();

  for(int e = 0; e < arrayLength; e++){

    if(pieces[e].getWhite() != white){
      if(pieces[e].getXpos() != 9 && pieces[e].getYpos() != 9){
        if(pieces[e].getType() == 'P'){

          if(pieces[e].getWhite()){


            if(pieces[e].getXpos() > 0 && pieces[e].getXpos() < 8 && pieces[e].getYpos() < 8){

              // white pawn diagonal right
              possibleMovescX.push_back(pieces[e].getXpos() + 1);
              possibleMovescY.push_back(pieces[e].getYpos() - 1);
              //white pawn diagonal left
              possibleMovescX.push_back( pieces[e].getXpos() - 1);
              possibleMovescY.push_back( pieces[e].getYpos() - 1);

            }else if(pieces[e].getXpos() == 0 && pieces[e].getYpos() < 8){
              // white pawn diagonal right
              possibleMovescX.push_back(pieces[e].getXpos() + 1);
              possibleMovescY.push_back(pieces[e].getYpos() - 1);

            }else if(pieces[e].getXpos() == 8 && pieces[e].getYpos() < 8){
              // white pawn diagonal right
              possibleMovescX.push_back(pieces[e].getXpos() - 1);
              possibleMovescY.push_back(pieces[e].getYpos() - 1);
            ...
            if(pieces[e].getXpos() > 0 && pieces[e].getXpos() < 8 && pieces[e].getYpos() > 0){
            ...
            }else if(pieces[e].getXpos() == 0 && pieces[e].getYpos() > 0){
              // white pawn diagonal right
              possibleMovescX.push_back(pieces[e].getXpos() + 1);
              possibleMovescY.push_back(pieces[e].getYpos() + 1);

            }else if(pieces[e].getXpos() == 8 && pieces[e].getYpos() > 0){
              // white pawn diagonal right
              possibleMovescX.push_back(pieces[e].getXpos() - 1);
              possibleMovescY.push_back(pieces[e].getYpos() + 1);
        ...
        }else if(pieces[e].getType() == 'Q'){

          for(int x1 = 0; x1 < 8; x1++){
            for(int y1 = 0; y1 < 8; y1++){
              if(Queen(e, x1,y1,false, true)){

                possibleMovescX.push_back(x1);
                possibleMovescY.push_back(y1);
              }
            }
          }

        }else if(pieces[e].getType() == 'R'){

          for(int x1 = 0; x1 < 8; x1++){
            for(int y1 = 0; y1 < 8; y1++){
              if(Rook(e, x1,y1,false, true)){
        ...
        }else if(pieces[e].getType() == 'B'){

          for(int x1 = 0; x1 < 8; x1++){
            for(int y1 = 0; y1 < 8; y1++){
              if(Bishop(e, x1,y1,false, true)){
        ...
        }else if(pieces[e].getType() == 'H'){

          for(int x1 = 0; x1 < 8; x1++){
            for(int y1 = 0; y1 < 8; y1++){
              if(Horse(e, x1,y1,false, true)){
        ...
        }else if(pieces[e].getType() == 'K'){

          for(int x1 = 0; x1 < 8; x1++){
            for(int y1 = 0; y1 < 8; y1++){
              if(King(e, x1,y1,false, true)){
  ...
  for (int e = 0; e < possibleMovescX.size(); e++){

    if(white){
      if (pieces[25].getXpos() == possibleMovescX[e] && pieces[25].getYpos() == possibleMovescY[e]){
        return true;
      }
    }else{
      if(pieces[9].getXpos() == possibleMovescX[e] && pieces[9].getYpos() == possibleMovescY[e]){
        return true;
      }
    }
  ...
  for(int o = 0; o < arrayLength; o++){
    if(pieces[o].getXpos() != 9 && pieces[o].getYpos() != 9){
      if(pieces[o].getWhite()){
        if(pieces[o].getType() == 'P' ){
          whitep += 1;
        }else if(pieces[o].getType() == 'K' ){
          whitek += 1;
        }else if(pieces[o].getType() == 'Q' ){
          whiteq += 1;
        }else if(pieces[o].getType() == 'R' ){
          whiter += 1;
        }else if(pieces[o].getType() == 'B' ){
          whiteb += 1;
        }else if(pieces[o].getType() == 'H' ){
          whiteh += 1;
        }
...
void AI(bool white) {
  ...
  if(aiLevel == 1){
    ...
    aiEval = 300;
    possibleMoves(false);
    // for(int ds = 0; ds < possibleMovesPiece.size(); ds++ ){
    //
    //   cout<<" piece "<< possibleMovesPiece[ds]<< endl;
    //   cout<<" Y "<< possibleMovesY[ds]<< endl;
    //   cout<<" x "<< possibleMovesX[ds]<< endl;
    //
    // }
    possibleMoves1X.clear();
    possibleMoves1Y.clear();
    possibleMoves1Piece.clear();
    possibleMoves1X = possibleMovesX;
    possibleMoves1Y =  possibleMovesY;
    possibleMoves1Piece = possibleMovesPiece;
    bestMove.clear();
    displacedX.clear();
    displacedY.clear();
    displacedPiece.clear();
    aiFirst = false;
    //  for(int z = 0; z <  possibleMovesPiece.size() ; z++){

    //    cout<< "possible moves " << possibleMovesY[z] << endl;

    //  }
    //  for(int z = 0; z <  possibleMoves1Piece.size() ; z++){

    //  cout<< " moves " << possibleMoves1Y[z] << endl;
    ...
    for(int z = 0; z <  possibleMoves1Piece.size() ; z++){
      ...
      if(pieces[possibleMoves1Piece[z]].getWhite() == white){
        ...
        if(pieces[possibleMoves1Piece[z]].getXpos() != 9 && pieces[possibleMoves1Piece[z]].getYpos() != 9){
          oldX1 = pieces[possibleMoves1Piece[z]].getXpos();

          oldY1 = pieces[possibleMoves1Piece[z]].getYpos();

          if(pieces[possibleMoves1Piece[z]].getFirst()){
            aiFirst = true;
          }else{
          ...
          for(int r = 0; r < arrayLength; r++){
            ...
            if(pieces[r].getXpos() == possibleMoves1X[possibleMovesX[z]] && pieces[r].getYpos() == possibleMoves1Y[possibleMovesY[z]]){
              ...
              displacedPiece.push_back(r);
              pieces[r].setCoords(9,9);

            }

          }
          // cout<<" piece "<< possibleMoves1Piece[z]<< endl;
          // cout<<" Y "<< possibleMoves1Y[z]<< endl;
          // cout<<" x "<< possibleMoves1X[z]<< endl;
          // cout << "z " << z << endl;


          pieces[possibleMoves1Piece[z]].setCoords( possibleMoves1X[z],possibleMoves1Y[z]);
          pieces[possibleMoves1Piece[z]].setFirst();
          // cout << "eval "<<evaluateBoard() << endl;
          // cout << "aieval "<<aiEval << endl;
          if(check(white) == false){
            if( evaluateBoard() < aiEval ){
              bestMove.clear();
              aiEval = evaluateBoard();
              bestMove.push_back(z);
            }else if(evaluateBoard() == aiEval ) {
              aiEval = evaluateBoard();
              bestMove.push_back(z);
            }
          }
          ...
          if(aiFirst){
            pieces[possibleMoves1Piece[z]].resetFirst();
          }
        }
      }

      for(int r = 0; r < displacedPiece.size(); r++){

        pieces[displacedPiece[r]].setCoords(displacedX[r],displacedY[r]);

      }

    }

    // cout<< "bestMove " << bestMove.size()<< endl;
...
void passTurn(){
  cout<< "checkmate "<< checkMate(false) << endl;
  if (whiteTurn == true){
    if(checkMate(false)){
      playing = false;
